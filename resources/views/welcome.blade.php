    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        @media (prefers-reduced-motion: reduce) {
            .mesh-blob, .dashboard-float, .ticker-track, .reveal {
                animation: none;
                opacity: 1;
                transform: none;
            }
...
<section id="pricing" class="py-24 px-6 bg-gray-50">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid md:grid-cols-{{ $plans->count() }} gap-8 items-stretch">
            @foreach($plans as $index => $plan)
                @php $popular = $index === 1; @endphp
                <div class="reveal {{ $popular ? 'pricing-popular rounded-3xl p-1 shadow-2xl shadow-indigo-500/30' : '' }}">
                    <div class="rounded-3xl {{ $popular ? 'bg-white h-full' : 'bg-white border border-gray-200 shadow-sm' }} p-8 relative flex flex-col">
                        @if($popular)
                        ...
                        <ul class="space-y-4 mb-8 flex-1">
                            @foreach([
                                ...
                                $plan->max_locations . ' warehouse ' . Str::plural('location', $plan->max_locations),
